...
folder_path <- "..\\data\\080807_WearMillingY_50%\\Level_010\\"
...
The value of $m_c$ must be in a range of *[0,1]*. Therefore a beta distribution is defined for this parameters. For the value of $k_{c11}$ a normal distribution is considered. The left hand side graphic shows the prior distruibution for $m_c$, the rigth hand side graphic the prior distribution for $k_{c11$.
...
The purpose of the FFT is to Identify dominant frequencies, phases, and amplitudes in a signal.
It reduces computational complexity from $𝑂(𝑁2)O(N2)$ in a direct DFT to $O(NlogN)$. 
Applications: Widely used in signal processing, image analysis, audio engineering, and system modeling.
For example, an FFT can analyze vibrations in machinery to detect faults or process audio signals for noise reduction.
The band pass filtered signal and the standard scalar preprocessed signal it transformed into the FFT and the ampliutudes arround the frquencies of 35Hz and 105 Hz are exatracted. The following graphic illustrated the the results for both signals.
...
data_mill_scsc <- list("k" = length(std_sc_amp4jags2eval), 
...
std_sc_jags_kienzle_McMax <- jags(data_mill, inits, parameters, model.file="jags_kienzle_McMaxL.txt", 
                      n.iter=5000, n.chains=20, n.burnin=1000)
```

```{r jags model evaluation and results printing (Likelihood)}
mc_post_mean <- std_sc_jags_kienzle_McMax$BUGSoutput$mean$mc
mc_post_sd<- std_sc_jags_kienzle_McMax$BUGSoutput$sd$mc
mc_deviance <- std_sc_jags_kienzle_McMax$BUGSoutput$mean$deviance
...
kc11_post_mean <- std_sc_jags_kienzle_McMax$BUGSoutput$mean$kc11
kc11_post_sd <- std_sc_jags_kienzle_McMax$BUGSoutput$sd$kc11
...
std_sc_jags_kienzle_McMax <- jags(data_mill, inits, parameters, model.file="jags_kienzle_McMaxLL.txt", 
                      n.iter=5000, n.chains=20, n.burnin=1000)
```
...
```{r run jags model (LogLikelihood)}
bp_jags_kienzle_McMax <- jags(data_mill_bp, inits, parameters, model.file="jags_kienzle_McMaxLL.txt", 
                      n.iter=5000, n.chains=20, n.burnin=1000)
```
...
```{r run jags model (Likelihood)}
bp_jags_kienzle_McMax <- jags(data_mill_bp, inits, parameters, model.file="jags_kienzle_McMaxL.txt", 
                      n.iter=5000, n.chains=20, n.burnin=1000)
```


### STAN Model
The Stan model is a statistical framework developed for advanced Bayesian modeling and inference. It is built on the Stan probabilistic programming language, which allows users to define complex probabilistic models through a structured syntax. The key strengths of Stan include its efficiency and flexibility, making it ideal for applications in regression, hierarchical modeling, time series analysis, and more.
